...
case "$BROWSER" in
  "google-chrome" | "google-chrome-stable" | "google-chrome-beta" | "google-chrome-unstable" | "chromium-browser")
    ...
    ;;
  "firefox")
    D=`mozprofile --pref="media.navigator.permission.disabled:true" --pref="browser.dom.window.dump.enabled:true" --pref="browser.dom.window.dump.file:${ERRCONSOLE}"`
    ;;
esac
...
while ! grep -q "${COND}" $ERRCONSOLE && browser_pids|grep -q .; do
  sleep 0.1
done
